...
{
    // temperature from the module was
...
{
    // PID from module, was
    float val = (float)((cmd.data[1] << 24) | (cmd.data[2] << 16) | (cmd.data[3] << 8) | cmd.data[4]) / 1'000;
...
ErrCode ToolHead3DP::Init(MAC_t &mac, uint8_t mac_index)
{
    ...
    UpdateHotendMaxTemp(275, 0);

    // we have configured 3DP in the same port
...
ErrCode ToolHead3DP::SetPID(uint8_t index, float value, uint8_t extrude_index)
{
    ...
    uint8_t  buffer[5];
    uint32_t scale_val = (uint32_t)(value * 1'000);
...
void ToolHead3DP::UpdateEAxisStepsPerUnit(ModuleToolHeadType type)
{
    ...
    {
        LOG_I(
            "found invalid E lead[%.3f], reset it to [%.3f]\n",
...
void ToolHead3DP::UpdateHotendMaxTemp(int16_t temp, uint8_t e /* = 0*/)
{
    ...
    {
    case 0:
        Temperature::temp_range[0].maxtemp = temp + HOTEND_OVERSHOOT;
        break;
    case 1:
        Temperature::temp_range[1].maxtemp = temp + HOTEND_OVERSHOOT;
        break;
    case 3:
        Temperature::temp_range[2].maxtemp = temp + HOTEND_OVERSHOOT;
        break;
    default:
        LOG_E("invalid extruder index: %d\n", e);
    }
}
...
void ToolHead3DP::SetTemp(int16_t temp, uint8_t extrude_index)
{
    ...
    if((cur_temp_[0] / 10) > Temperature::temp_range[0].maxtemp)
    {
        systemservice.ThrowException(EHOST_HOTEND0, ETYPE_OVERRUN_MAXTEMP);
    }
}

void ToolHead3DP::NozzleFanCtrlCheck()
{
    ...
    {
        if(thermalManager.degTargetHotend(i) >= NOZZLE_FAN_AUTO_ENABLE_TEMP
           || thermalManager.degHotend(i) >= NOZZLE_FAN_AUTO_ENABLE_TEMP)
        {
            enable_fan = 1;
        }
    }
    ...
    {
        if((enable_fan == 0 && fan_speed_[nozzle_fan_index])
           || (enable_fan == 1 && fan_speed_[nozzle_fan_index] != 255))
        {
