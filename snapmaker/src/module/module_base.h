 ...
 * in the future, there will be some new module plugged in system, and the controller doesn't
...
public:
    ...
    static ModuleToolHeadType toolhead()
    {
        return toolhead_;
    }
    ...
    {
        return lock_marlin_uart_;
    };
    static LockMarlinUartSource lock_marlin_source()
    {
        return lock_marlin_source_;
    };
    ...
    virtual ErrCode Init(MAC_t &mac, uint8_t mac_index)
    {
        return E_SUCCESS;
    }
    virtual ErrCode PostInit()
    {
        return E_SUCCESS;
    ...
    virtual bool IsOnline(uint8_t sub_index = 0)
    {
        return false;
    }

    virtual uint32_t mac(uint8_t sub_index = 0)
    {
